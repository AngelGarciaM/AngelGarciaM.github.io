...
def abrir_correcion():
   ...
   # Crear una lista de diccionarios de EvPDF
    ...
    resutados_path=os.path.join(basedir, 'app', 'tmp', f'resultados_{current_user.usuario}.json')
...
def abrir_cuestionario():
    ...
    basedir = os.path.abspath(os.path.dirname(__file__))
    temp_file_path = os.path.join(basedir, 'app', 'tmp','temp_data_cuestionario_{current_user.usuario}.json')
    ...
    resultados_path = os.path.join(basedir, 'app', 'tmp', f'resultados_cuestionario_{current_user.usuario}.json')
