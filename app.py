...
from flask import Flask, render_template, request, redirect, url_for,g, make_response, flash, session, jsonify
...
@app.route('/abrir_correcion', methods=['POST', 'GET'])
@login_required
def abrir_correcion():
    if request.method == 'POST':
        data = request.get_json()
        correccion_nombre = data.get('nombre_correccion', '')
        lista_titulo=data.get('lista_titulo','')
        if "cancelar" in data and data["cancelar"] == "cancelar":
            lista_evpdf=[]
            #Si se cancela, se vuelve a poner la lista en False para que se pueda corregir y se vuelve a mostrar todas las correcciones
            with lock:
            ...
            return jsonify({'status': 'cancel', 'redirect': '/correcciones'})  # Aquí se devuelve la URL de redirección
        ranking_final = data.get('ranking_final', [])
        lista_evpdf=[]
        for data in ranking_final:
            evpdf = EvPDF(namePdf=data['namePdf'], texto=data['path'])
            lista_evpdf.append(evpdf)
        
        i=0
        con=len(lista_evpdf)
        for i in range(0,len(lista_evpdf)):
            lista_evpdf[i].nota=con
            con-=1
        ...
        with lock:
            ...
            socketio.emit('actualizar_pagina') #mensaje que capturan todos los clientes y actualizan la página
            return jsonify({'status': 'success', 'redirect': '/correcciones'})  # Aquí se devuelve la URL de redirección
    ...
    if request.method == 'GET':
        ...
        return render_template('comparaciones_profesores.html', pdfs=evpdf_data, nombre_correccion=correccion_nombre, lista_titulo=lista_titulo)

        
@app.route('/abrir_cuestionario', methods=['POST', 'GET'])
@login_required
def abrir_cuestionario():
    ...
    if not os.path.exists(tmp_dir):
        os.makedirs(tmp_dir)
    temp_file_path = os.path.join(tmp_dir,f'temp_data_cuestionario_{current_user.usuario}.json')
    
    if request.method == 'POST':
        
        # Obtener los datos de texto del formulario
        nombre_correccion = request.form.get('nombre_correcion')  # Campo de texto
        tipo = request.form.get('tipo')  # Tipo de corrección (general o privado)
        num_corr = request.form.get('numero_correctores')  # Número de correctores
        lista_correctores = request.form.getlist('correctores_seleccionados[]')  # Correctores seleccionados

        # Imprimir los datos del formulario para verificar
        print(f"Nombre Corrección: {nombre_correccion}")
        print(f"Tipo de corrección: {tipo}")
        print(f"Número de correctores: {num_corr}")
        print(f"Correctores seleccionados: {lista_correctores}")

        # Obtener los archivos enviados
        archivos = request.files.getlist('pdfs')  # Lista de archivos
        lista_evpdf=analizar_archivos(archivos)
        ...
        dic_listas=distribute_reviews_uniform(lista_evpdf,lista_correctores)
        lista_listas=[]
        ...
        with lock:
            tutores = g.db_manager.obtener_nombres_todos_tutores()
            tutores = [tutor[0] for tutor in tutores]
        return render_template('formulario.html', correcciones=tutores, ruta_json=temp_file_path)

#Devuelve una lista de EvPDF
def analizar_archivos(archivos):
    lista_evpdf=[]
    for pdf in archivos:
        try:
            # Leer el archivo PDF desde el objeto file en memoria
            ...
            for page_num in range(doc.page_count):
                ...
                text += page.get_text()  # Obtener el texto de la página
            
            # Ya tenemos el texto, lo convertimos a EvPDF y lo añadimos a la lista
            lista_evpdf.append(EvPDF(pdf.filename,text))
